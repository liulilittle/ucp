...
namespace UcpTest
{
    ...
    internal sealed class UcpPerformanceReport
    {
        ...
        public double CurrentBandwidthBytesPerSecond;
        ...
        {
            get { return ToMbps(CurrentBandwidthBytesPerSecond); }
        }
        ...
        public static UcpPerformanceReport FromConnection(string scenarioName, UcpConnection connection, double throughputBytesPerSecond, long elapsedMilliseconds, double targetBandwidthBytesPerSecond, double observedLossPercent)
        {
            ...
            performance.CurrentBandwidthBytesPerSecond = report.MeasuredBandwidthBytesPerSecond;
            ...
            performance.BandwidthWastePercent = 0 == report.DataPacketsSent ? 0 : report.RetransmittedPackets * 100d / report.DataPacketsSent;
        ...
        public static void Append(string filePath, UcpPerformanceReport report)
        {
            ...
            lock (Sync)
            {
                ...
                builder.AppendLine("Current(Mbps): " + report.CurrentMbps.ToString("F2"));
                ...
                builder.AppendLine();
                reportEntry = builder.ToString();
            }

            File.AppendAllText(filePath, reportEntry, Encoding.UTF8);
        ...
        public static string BuildConsoleTable()
        {
            ...
            builder.AppendLine("Notes: Current Mbps is ACK-confirmed delivery rate; Pacing Mbps is the controller send ceiling.");
        ...
        public static bool ValidateReportFile(string filePath, out string errorMessage)
        {
            ...
            for (int i = 0; i < parsedReports.Count; i++)
            {
                ...
                if (report.ForwardDelayMicros > 0 && report.ReverseDelayMicros > 0)
                {
                    ...
                    if (delayDiffMicros > 15 * UcpConstants.MICROS_PER_MILLI)
                    {
                        errorMessage = "Scenario " + report.ScenarioName + " directional delay difference exceeds the 15ms route-skew limit.";
                        return false;
                    }
                ...
                {
                    hasLongFatPipe = true;
                    if (report.RetransmissionRatio > 0.05d || (report.PacingRateBytesPerSecond > 0 && report.PacingRateBytesPerSecond < report.TargetBandwidthBytesPerSecond * 0.30d) || report.UtilizationPercent < 80d)
                    {
                ...
                {
                    hasHighJitter = true;
                    if (report.UtilizationPercent <= 15d || report.RetransmissionRatio > UcpConstants.DEFAULT_MAX_BANDWIDTH_LOSS_PERCENT / 100d)
                    {
            ...
            if (!hasForwardHigherDelay || !hasReverseHigherDelay)
            {
                if (hasForwardHigherDelay || hasReverseHigherDelay)
                {
                    Console.WriteLine("VALIDATION NOTE: Only " + (hasForwardHigherDelay ? "forward-heavy" : "reverse-heavy") + " delays detected (expected for unidirectional tests).");
                }
            }
        ...
        private static void AppendTable(StringBuilder builder, List<UcpPerformanceReport> snapshot, bool includeNotes)
        {
            ...
            string[] headers = new string[] { "Scenario", "Throughput Mbps", "Target Mbps", "Util%", "Retrans%", "Loss%", "A->B ms", "B->A ms", "Avg ms", "P95 ms", "P99 ms", "Jit ms", "CWND", "Current Mbps", "Pacing Mbps", "RWND", "Waste%", "Conv" };
            ...
            for (int i = 0; i < snapshot.Count; i++)
            {
                ...
                rows.Add(new string[]
                {
                    ...
                    FormatDouble(report.CurrentMbps),
        ...
        private static List<UcpPerformanceReport> ParseReportFile(string filePath)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                string column13Header = headerColumns.Length > 14 ? headerColumns[14].Trim() : string.Empty;
                ...
                if (columns.Length >= 20)
                {
                    ...
                    report.CurrentBandwidthBytesPerSecond = FromMbps(ParseDouble(columns[14]));
                    report.PacingRateBytesPerSecond = FromMbps(ParseDouble(columns[15]));
                    report.RemoteWindowBytes = (uint)Math.Round(ParseByteSize(columns[16]));
                    report.BandwidthWastePercent = ParseDouble(columns[17]);
                    report.ConvergenceMilliseconds = ParseTimeDisplay(columns[18]);
                }
                else if (columns.Length >= 19)
                {
                    ...
                    report.JitterMicros = FromMilliseconds(ParseDouble(columns[12]));
                    report.CongestionWindowBytes = (int)Math.Round(ParseByteSize(columns[13]));
                    if (string.Equals(column13Header, "Current Mbps", StringComparison.OrdinalIgnoreCase))
                    {
                        report.CurrentBandwidthBytesPerSecond = FromMbps(ParseDouble(columns[14]));
                    }
                    else
        ...
        {
            return bytesPerSecond * UcpBenchmarkConstants.BITS_PER_BYTE / UcpBenchmarkConstants.BITS_PER_MEGABIT;
        }

        private static double FromMbps(double megabitsPerSecond)
        {
            return megabitsPerSecond * UcpBenchmarkConstants.BITS_PER_MEGABIT / UcpBenchmarkConstants.BITS_PER_BYTE;
        }
        ...
        private static int GetScenarioOrder(string scenarioName)
        {
            ...
            if (scenarioName == "100M_Loss0.2")
            {
                return 220;
            }

            if (scenarioName == "100M_Loss1")
            {
                return 230;
            }

            if (scenarioName == "100M_Loss10")
            {
                return 240;
            }

            if (scenarioName == "1G_Loss3")
            {
                return 250;
            }

            if (scenarioName == "AirplaneWifi")
            {
                return 260;
            }

            if (scenarioName == "HighSpeedTrain")
            {
                return 270;
            }

            if (scenarioName == "DrivingVehicle")
            {
                return 280;
            }
