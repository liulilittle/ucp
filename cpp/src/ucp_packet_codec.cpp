...
uint16_t UcpPacketCodec::ReadUInt16(const uint8_t* buffer, size_t offset) noexcept {
    return static_cast<uint16_t>((static_cast<uint16_t>(buffer[offset]) << BYTE_BITS) | buffer[offset + 1]);
}

void UcpPacketCodec::WriteUInt16(uint16_t value, uint8_t* buffer, size_t offset) noexcept {
    buffer[offset] = static_cast<uint8_t>(value >> BYTE_BITS);
    buffer[offset + 1] = static_cast<uint8_t>(value);
}

uint32_t UcpPacketCodec::ReadUInt32(const uint8_t* buffer, size_t offset) noexcept {
    return (static_cast<uint32_t>(buffer[offset]) << UINT24_BITS) | (static_cast<uint32_t>(buffer[offset + 1]) << UINT16_BITS) |
           (static_cast<uint32_t>(buffer[offset + 2]) << BYTE_BITS) | buffer[offset + 3];
}

void UcpPacketCodec::WriteUInt32(uint32_t value, uint8_t* buffer, size_t offset) noexcept {
...
uint64_t UcpPacketCodec::ReadUInt48(const uint8_t* buffer, size_t offset) noexcept {

    uint64_t value = (static_cast<uint64_t>(buffer[offset]) << UINT40_BITS) | (static_cast<uint64_t>(buffer[offset + 1]) << UINT32_BITS) |
                     (static_cast<uint64_t>(buffer[offset + 2]) << UINT24_BITS) |
                     (static_cast<uint64_t>(buffer[offset + 3]) << UINT16_BITS) | (static_cast<uint64_t>(buffer[offset + 4]) << BYTE_BITS) |
                     buffer[offset + 5];
    return value;
}

void UcpPacketCodec::WriteUInt48(uint64_t value, uint8_t* buffer, size_t offset) noexcept {
...
uint64_t UcpPacketCodec::ReadUInt64(const uint8_t* buffer, size_t offset) noexcept {
    return (static_cast<uint64_t>(buffer[offset]) << UINT56_BITS) | (static_cast<uint64_t>(buffer[offset + 1]) << UINT48_BITS) |
           (static_cast<uint64_t>(buffer[offset + 2]) << UINT40_BITS) | (static_cast<uint64_t>(buffer[offset + 3]) << UINT32_BITS) |
           (static_cast<uint64_t>(buffer[offset + 4]) << UINT24_BITS) | (static_cast<uint64_t>(buffer[offset + 5]) << UINT16_BITS) |
           (static_cast<uint64_t>(buffer[offset + 6]) << BYTE_BITS) | buffer[offset + 7];
}

void UcpPacketCodec::WriteUInt64(uint64_t value, uint8_t* buffer, size_t offset) noexcept {
    buffer[offset] = static_cast<uint8_t>(value >> UINT56_BITS);
    buffer[offset + 1] = static_cast<uint8_t>(value >> UINT48_BITS);
    buffer[offset + 2] = static_cast<uint8_t>(value >> UINT40_BITS);
    buffer[offset + 3] = static_cast<uint8_t>(value >> UINT32_BITS);
    buffer[offset + 4] = static_cast<uint8_t>(value >> UINT24_BITS);
    buffer[offset + 5] = static_cast<uint8_t>(value >> UINT16_BITS);
    buffer[offset + 6] = static_cast<uint8_t>(value >> BYTE_BITS);
    buffer[offset + 7] = static_cast<uint8_t>(value);
}

bool UcpPacketCodec::TryReadCommonHeader(const uint8_t* buffer, size_t offset, size_t count, UcpCommonHeader& header) noexcept {
...
void UcpPacketCodec::WriteCommonHeader(const UcpCommonHeader& header, uint8_t* buffer, size_t offset) noexcept {
...
ucp::vector<uint8_t> UcpPacketCodec::Encode(const UcpPacket& packet) noexcept {
...
bool UcpPacketCodec::TryDecode(const uint8_t* buffer, size_t offset, size_t count, ucp::shared_ptr<UcpPacket>& out_packet) noexcept {
    out_packet = NULLPTR;
    if (NULLPTR == buffer || count < COMMON_HEADER_SIZE) {
        return false;
    }
    ...
    case UcpPacketType::Rst: {
        ...
        auto control = ucp::make_shared_object<UcpControlPacket>();
        if (NULLPTR == control) {
            return false;
        }
        ...
        bool hasAck = HasAckNumber == (header.flags & HasAckNumber);
        ...
        if (count >= controlIndex + SEQUENCE_NUMBER_SIZE) {
            ...
            controlIndex += SEQUENCE_NUMBER_SIZE;
        }

        if (count >= controlIndex + SESSION_KEY_SIZE) {
            control->session_key = ReadUInt64(buffer, controlIndex);
        }
...
ucp::vector<uint8_t> UcpPacketCodec::EncodeData(const UcpDataPacket& packet) noexcept {
    size_t payloadLength = packet.payload.size();
    bool hasAck = HasAckNumber == (packet.header.flags & HasAckNumber);
    size_t blockCount = 0;
    ...
    ucp::vector<uint8_t> bytes(baseHeaderSize + blockCount * SACK_BLOCK_SIZE + payloadLength);
    size_t index = 0;
...
bool UcpPacketCodec::TryDecodeData(const uint8_t* buffer, size_t offset, size_t count, const UcpCommonHeader& header,
    ...
    auto data = ucp::make_shared_object<UcpDataPacket>();
    if (NULLPTR == data) {
        return false;
    }
    data->header = header;
    ...
    if (hasAck) {
        ...
        index += sizeof(uint16_t);
        if (blockCount > static_cast<uint16_t>(MAX_ACK_SACK_BLOCKS)) {
            blockCount = static_cast<uint16_t>(MAX_ACK_SACK_BLOCKS);
        }
...
ucp::vector<uint8_t> UcpPacketCodec::EncodeAck(const UcpAckPacket& packet) noexcept {
    ...
    ucp::vector<uint8_t> bytes(ACK_FIXED_SIZE + blockCount * SACK_BLOCK_SIZE);
    size_t index = 0;
...
bool UcpPacketCodec::TryDecodeAck(const uint8_t* buffer, size_t offset, size_t count, const UcpCommonHeader& header,
                                  ucp::shared_ptr<UcpPacket>& out_packet) noexcept {
    out_packet = NULLPTR;
    ...
    size_t index = offset + COMMON_HEADER_SIZE;
    auto ack = ucp::make_shared_object<UcpAckPacket>();
    if (NULLPTR == ack) {
        return false;
    }
    ack->header = header;
...
ucp::vector<uint8_t> UcpPacketCodec::EncodeNak(const UcpNakPacket& packet) noexcept {
    size_t count = packet.missing_sequences.size();
    if (count > static_cast<size_t>(MAX_NAK_SEQUENCES_PER_PACKET)) {
        count = static_cast<size_t>(MAX_NAK_SEQUENCES_PER_PACKET);
    }
    ucp::vector<uint8_t> bytes(NAK_FIXED_SIZE + count * SEQUENCE_NUMBER_SIZE);
    size_t index = 0;
...
bool UcpPacketCodec::TryDecodeNak(const uint8_t* buffer, size_t offset, size_t count, const UcpCommonHeader& header,
                                  ucp::shared_ptr<UcpPacket>& out_packet) noexcept {
    out_packet = NULLPTR;
    ...
    size_t index = offset + COMMON_HEADER_SIZE;
    auto nak = ucp::make_shared_object<UcpNakPacket>();
    if (NULLPTR == nak) {
        return false;
    }
    nak->header = header;
    ...
    index += sizeof(uint16_t);
    if (missingCount > static_cast<uint16_t>(MAX_NAK_SEQUENCES_PER_PACKET)) {
        missingCount = static_cast<uint16_t>(MAX_NAK_SEQUENCES_PER_PACKET);
    }
...
ucp::vector<uint8_t> UcpPacketCodec::EncodeFecRepair(const UcpFecRepairPacket& packet) noexcept {
    size_t payloadLen = packet.payload.size();
    ucp::vector<uint8_t> bytes(COMMON_HEADER_SIZE + sizeof(uint32_t) + sizeof(uint8_t) + payloadLen);
    ...
    if (0 < payloadLen) {
        std::memcpy(bytes.data() + COMMON_HEADER_SIZE + sizeof(uint32_t) + sizeof(uint8_t), packet.payload.data(), payloadLen);
    }

    return bytes;
}

bool UcpPacketCodec::TryDecodeFecRepair(const uint8_t* buffer, size_t offset, size_t count, const UcpCommonHeader& header,
                                        ucp::shared_ptr<UcpPacket>& out_packet) noexcept {
    out_packet = NULLPTR;
    ...
    auto repair = ucp::make_shared_object<UcpFecRepairPacket>();
    if (NULLPTR == repair) {
        return false;
    }
    ...
    if (0 < payloadLen) {
        ...
        std::memcpy(repair->payload.data(), buffer + offset + fecFixedSize, payloadLen);
    }
...
ucp::vector<uint8_t> UcpPacketCodec::EncodeControl(const UcpControlPacket& packet) noexcept {
    bool hasAck = HasAckNumber == (packet.header.flags & HasAckNumber);
    ...
    if (0 != packet.session_key) {
        size += SESSION_KEY_SIZE;
    }

    ucp::vector<uint8_t> bytes(size);
    size_t index = 0;
    WriteCommonHeader(packet.header, bytes.data(), index);
    index += COMMON_HEADER_SIZE;
    ...
    if (0 != packet.session_key) {
        WriteUInt64(packet.session_key, bytes.data(), index);
    }
