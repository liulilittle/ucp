...
| [架构](architecture_CN.md) | [Architecture Comprehensive Overview](architecture.md) |
...
| [performance_CN.md](performance_CN.md) | 中文 | 性能基准框架总览、14+场景分类矩阵(稳定无丢包/随机丢包/长肥管/非对称路由/弱移动网络/突发丢包)、基准结果数据表、报告16列字段口径详述、Retrans%与Loss%独立性评估及Mermaid图、7条校验规则(吞吐封顶/重传合法范围/方向延迟差/损失独立计算/收敛非零/CWND非零)、方向路由非对称建模(Mermaid图)、端到端丢包恢复完整流程图、BBRv2拥塞恢复策略参数表、性能调优指南(MSS/发送缓冲/FEC/常见陷阱)、验收标准矩阵 |
...
| [constants_CN.md](constants_CN.md) | 中文 | `UcpConstants`所有常量的详尽目录：包编码(9项)、RTO与恢复定时器(8项)、Pacing与队列(5项)、快重传与NAK分级(14项含SACK 5项+NAK 9项)、BBRv2恢复参数(7项)、前向纠错(10项含组大小/GF256多项式/自适应阈值)、基准Payload选择(12场景+原因)、基准验收标准(5项)、报告收敛说明器格式表、路由与弱网常量(6项)、连接与会话常量(4项)。合计77+个协议常量 |
