...
这是 UCP 文档的全局入口。中文文档使用 `_CN` 后缀；每个中文页面都链接到对应英文页面，便于双语维护团队协作。文档按五个纵向切片组织，每部分覆盖协议的一个独特方面：
...
| 文档 | 作用 |
|---|---|
| [../README.md](../README.md) | 项目概览、快速开始、功能矩阵和报告字段摘要，面向首次使用者。 |
| [performance_CN.md](performance_CN.md) | 14 个场景的基准矩阵、报告字段口径、校验规则、方向路由建模、端到端丢包恢复流程图和 BBRv2 吞吐调优策略。 |
| [protocol_CN.md](protocol_CN.md) | 权威线格式规范，含所有包类型捎带累积 ACK、QUIC 风格 SACK 配每块发送限制、NAK 分级置信度恢复（低/中/高三级）、BBRv2 自适应 pacing 增益拥塞控制、FEC Reed-Solomon GF(256) 自适应传输以及完整连接状态机。 |
| [architecture_CN.md](architecture_CN.md) | 内部运行时分层（从应用 API 到传输 socket）、基于连接 ID 的会话追踪（IP 无关、随机 ConnId）、每连接随机 ISN 生成、服务端动态 IP 绑定、每连接 SerialQueue 串行执行模型、带 credit 记账的服务端公平队列调度、pacing controller token bucket 设计、BBRv2 投递率估计内核以及确定性网络模拟器架构。 |
...
| [api_CN.md](api_CN.md) | 完整公开 API 接口文档，含 `UcpConfiguration.GetOptimizedConfig()` 工厂方法及所有参数默认值、`UcpServer` 生命周期（Start/AcceptAsync/Stop）、`UcpConnection` 连接管理/发送/接收/事件/诊断方法、`UcpNetwork.DoEvents()` 驱动循环、`ITransport` 自定义传输接口以及含错误处理的完整端到端代码示例。 |
...
flowchart TD
    ...
    J --> K[确认零 report-error 行]
    K --> L{有失败?}
    L -->|是| M[修复协议或测试]
    L -->|否| N[更新 CHANGELOG 并提交]
    M --> H
```
...
| `test_report.txt` | `Ucp.Tests/bin/Debug/net8.0/reports/` | 由 `ReportPrinter` 校验的标准 ASCII 汇总表。包含所有 14 个场景行，含吞吐、利用率、重传率、方向 RTT 统计、CWND、pacing 速率和收敛时间。 |
